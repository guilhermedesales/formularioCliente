document
  ...
  .addEventListener("submit", function (e) {
    ...
    const primeiraMenstruacao = document.getElementById(
      "primeiraMenstruacao"
    ).value;
    ...
    */
    ...
    const gravida =
      document.querySelector('input[name="gravida"]:checked')?.value === "Sim";
    ...
    if (gravida) {
      const qtdGravidez =
        parseInt(document.getElementById("qtd-gravidez")?.value?.trim()) || 0;
      const partoNormal =
        parseInt(document.getElementById("parto-normal")?.value?.trim()) || 0;
      const cesariana =
        parseInt(document.getElementById("cesariana")?.value?.trim()) || 0;
      const aborto =
        parseInt(document.getElementById("aborto")?.value?.trim()) || 0;
      ...
      const ligadura =
        document.querySelector('input[name="ligadura tubaria"]:checked')
          ?.value || "Não informado";
      const complicacoesVal = document
        .getElementById("complicacoes")
        ?.value?.trim();
    ...
    const cirurgiasGerais =
      document.getElementById("cirurgias")?.value || "Não informado";

    // Função auxiliar para obter valor de radio button
    ...
    function getValorCampoExtra(checkedName, inputName) {
      ...
      if (value === "Sim") {
        ...
        return extraInput?.value
          ? `Sim (${extraInput.value})`
    ...
    function getValorComCampoExtra(radioName, extraInputName) {
      ...
      if (val === "sim" || val === "Sim") {
        const extraInput = document.querySelector(
          `input[name="${extraInputName}"]`
        );
        return extraInput && extraInput.value.trim() !== ""
          ? `${val} (${extraInput.value.trim()})`
          : val;
      }
      return val;
    }
    ...
    const respiratoria = getValorCampoExtra(
      "respiratoria",
      "respiratoria_qual"
    );
    const cardiaca = getValorCampoExtra("cardiaca", "cardiaca_qual");
    const psiquiatrica = getValorCampoExtra(
      "psiquiatrica",
      "psiquiatrica_qual"
    );
    ...
    // Autoimunes
    const autoimunesMarcadas = Array.from(
      document.querySelectorAll('input[name="autoimune[]"]:checked')
    ).map((cb) => cb.value);
    ...
    if (autoimunesMarcadas.length > 0) {
      ...
      if (autoimunesMarcadas.includes("Outra")) {
        const descricao = document
          .querySelector('input[name="autoimune_outra_descricao"]')
          ?.value?.trim();
    ...
    const alergiaMedicamento =
      getValorRadio("alergia-medicamento").toLowerCase() === "sim";
    const quaisMedicamentos = alergiaMedicamento
      ? document
          .querySelector('input[name="quais-medicamentos"]')
          ?.value.trim() || ""
      : "";
    ...
    const parouFumar =
      fuma.toLowerCase() === "nao" || fuma.toLowerCase() === "sim"
        ? document.querySelector('input[name="parouFumar"]')?.value.trim() ||
          "Não informado"
        : "Ainda fuma";

    // Intolerância alimentar
    const intolerancia = getValorRadio("intolerancia");
    const descIntolerancia =
      intolerancia.toLowerCase() === "sim"
        ? document
            .querySelector('input[name="descIntolerancia"]')
            ?.value.trim() || "Não especificado"
        : "Não aplicável";

    // Hábito de tomar café
    const cafe = getValorRadio("cafe");
    const qtdCafe =
      cafe.toLowerCase() === "sim"
        ? document.querySelector('input[name="qtdCafe"]')?.value.trim() ||
          "Não especificado"
        : "Não";

    // Uso de medicação
    const medicacao =
      document.querySelector('textarea[name="Medicação"]')?.value.trim() ||
      "Nenhuma";
    const medicacaoAntes =
      document.querySelector('textarea[name="MedicaçãoAntes"]')?.value.trim() ||
      "Nenhuma";
    ...
    function getCheckboxWithDetail(checkboxName, detailFieldName) {
      ...
      if (checked) {
        const detail = document
          .querySelector(`input[name="${detailFieldName}"]`)
          ?.value.trim();
    ...
    function getCheckboxWithTextareaDetail(checkboxName, textareaName) {
      ...
      if (checked) {
        const text = document
          .querySelector(`textarea[name="${textareaName}"]`)
          ?.value.trim();
    ...
    const hf_cardiologica = getCheckboxWithDetail(
      "hf_cardiologica",
      "hf_cardiologica_qual"
    );
    const hf_cancer = getCheckboxWithDetail(
      "hf_cancer_checkbox",
      "hf_cancer_info"
    );
    const hf_outra = getCheckboxWithTextareaDetail(
      "hf_outra",
      "hf_outra_descricao"
    );
    ...
    const info_adicional =
      document
        .querySelector('textarea[name="informacao_adicional"]')
        ?.value.trim() || "";

    const texto = `*Dados Pessoais*%0A%0A- Nome: ${nome}%0A- Nome Social: ${nomeSocialFinal}%0A- Data de Nascimento: ${dataNascimento} (Idade: ${idade})%0A- Cor: ${cor}%0A- Estado Civil: ${estadoCivil}%0A- Profissão: ${profissao}%0A- CEP: ${cep}%0A- Endereço: ${endereco}%0A- Indicação: ${indicacao}%0A- E-mail: ${email}%0A- Telefones: ${telefones}%0A%0A*Informações Clínicas*%0A%0A- Altura: ${altura} / Peso: ${peso} / IMC: ${imc}%0A
...
%0A- HPP: ${
      [
        ...
        ist.toLowerCase().startsWith("sim")
          ? `IST (${ist.split("(")[1]?.replace(")", "") || "não especificada"})`
          : "",
        respiratoria.toLowerCase().startsWith("sim")
          ? `Doença Respiratória (${
              respiratoria.split("(")[1]?.replace(")", "") || "não especificada"
            })`
          : "",
        tireoidismo.toLowerCase() === "hiper"
          ? "Hipertireoidismo"
          : tireoidismo.toLowerCase() === "hipo"
          ? "Hipotireoidismo"
          : "",
        cardiaca.toLowerCase().startsWith("sim")
          ? `Doença Cardíaca (${
              cardiaca.split("(")[1]?.replace(")", "") || "não especificada"
            })`
          : "",
        psiquiatrica.toLowerCase().startsWith("sim")
          ? `Doença Psiquiátrica (${
              psiquiatrica.split("(")[1]?.replace(")", "") || "não especificada"
            })`
          : "",
        cancer.toLowerCase().startsWith("sim")
          ? `Câncer (${
              cancer.split("(")[1]?.replace(")", "") || "não especificado"
            })`
          : "",
...
- Fuma ou já fumou: ${fuma}${
      fuma.toLowerCase() === "sim" && parouFumar
        ? ` (Parou há: ${parouFumar})`
        : ""
    }%0A
- Intolerância alimentar: ${
...
%0A- HF: ${
      [
        ...
        hf_cardiologica.toLowerCase().startsWith("sim")
          ? `Doença Cardiológica (${
              hf_cardiologica.split("(")[1]?.replace(")", "") ||
              "não especificada"
            })`
          : "",
        hf_cancer.toLowerCase().startsWith("sim")
          ? `Câncer (${
              hf_cancer.split("(")[1]?.replace(")", "") || "não especificado"
            })`
          : "",
        hf_outra.toLowerCase().startsWith("sim")
          ? `${hf_outra.split("(")[1]?.replace(")", "") || "não especificada"}`
          : "",
      ]
        .filter(Boolean)
        .join(", ") || "Nenhuma condição relatada"
    }
