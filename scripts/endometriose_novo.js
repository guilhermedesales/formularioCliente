...
function showReviewModal() {
    ...
    html += buildReviewItem('Nome Social', formData.get('Nome Social') || 'Não informado');
    html += buildReviewItem('Data de Nascimento', formData.get('Data nascimento'));
    ...
    html += buildReviewItem('Telefone Secundário', formData.get('Telefone2') || 'Não informado');
    ...
    html += buildReviewItem('Primeira Menstruação', formData.get('Primeira Menstruação') + ' anos');
    html += buildReviewItem('Primeira Relação Sexual', formData.get('Primeira Relação') + ' anos');
    ...
    if (respiratoria === 'Sim') {
        html += buildReviewItem('Doença Respiratória', formData.get('respiratoria_qual') || 'Sim');
    } else {
    ...
    if (cardiaca === 'Sim') {
        html += buildReviewItem('Doença Cardíaca', formData.get('cardiaca_qual') || 'Sim');
    } else {
    ...
    if (psiquiatrica === 'Sim') {
        html += buildReviewItem('Doença Psiquiátrica', formData.get('psiquiatrica_qual') || 'Sim');
    } else {
    ...
    if (alergiaMed === 'sim') {
        ...
        html += buildReviewItem('Látex', formData.get('alergia-latex') === 'sim' ? 'Sim' : 'Não');
    } else {
    ...
    html += buildReviewItem('Condições Familiares', hfList.length > 0 ? hfList.join('; ') : 'Nenhuma');
    html += '</div>';
    ...
    if (fuma === 'sim') {
        ...
        html += buildReviewItem('Tabagismo', 'Já fumou' + (parou ? ' - parou há ' + parou : ''));
    ...
    if (intolerancia === 'sim') {
        html += buildReviewItem('Intolerância Alimentar', formData.get('descIntolerancia') || 'Sim');
    } else {
    ...
    html += buildReviewItem('Medicações Atuais', formData.get('Medicação') || 'Nenhuma');
    html += buildReviewItem('Medicações Anteriores', formData.get('MedicaçãoAntes') || 'Nenhuma');
    html += '</div>';
